...
import { getUserAddress, register, getCarsByStatus, getCar, getOwner, login } from "./Web3Client";
// Assets
  ...
  useEffect(() => {
    const handleInit = async () => {
      ...
      if (isAUser.address !== emptyAddress) {
        ...
        // set user credits
        setUserCredit(Web3.utils.fromWei(String(isAUser.balance), "ether").toString());
        // set user due
        let userDue = Web3.utils.fromWei(String(isAUser.debt), "ether").toString();
        setDue(userDue);
        // set user name
        if (isAUser.name) setUserName(isAUser.name);
        ...
        if (isAUser.rentedCarId !== "0") {
          ...
          setIsAvailable(`Rented ${rentedCar && rentedCar.name} - ${rentedCar && rentedCar.id}`);
        } else {
    ...
    <div className="h-full bg-[url('./assets/background.jpg')]  bg-cover bg-center  bg-no-repeat ">
      ...
      {loggedIn ? (
        <div>
          ...
          <div className="grid place-content-center mt-8">{isAdmin && <GradientButton onClick={() => setShowModal(true)} title="Admin Actions" />}</div>
          {/* Data Section */}
          <div className=" mx-auto grid place-content-center  mt-12">
            <div className="flex flex-row items-center">
              <div className="grid grid-flow-row md:grid-flow-col  items-center">
                <InfoBox label="BNB Credit" number={userCredit} icon={<BiWalletAlt />} />
                <InfoBox label="BNB Due" number={due} icon={<GiToken />} />
                <InfoBox label="Ride Minutes" number={rideMins} icon={<BiTimeFive />} />
          ...
          <div className="place-content-center  grid items-center p-4 mt-12">
            <InputComponent holder=" Credit balance" label="Credit your account" type="credit" />
            <DueComponent label="Pay your due" />
            <InputComponent holder=" Withdraw balance" label="Withdraw token from your account" type="withdraw" />
          </div>
          {/* Car Section */}
          <div className="grid md:grid-flow-col gap-4 gap-y-12 justify-evenly mt-24 pb-24">
            {cars.length > 0 ? (
              cars.map((car) => <div key={car && car.id}>{car && <CarComponent carStatus={car.status} rentFee={car.rentFee} saleFee={car.saleFee} image={car.imgUrl} id={car.id} name={car.name} />}</div>)
            ) : (
              <div>
        ...
        <>
          <div className="h-screen text-white  w-full">
            <div className=" p-4 mt-10 grid place-content-center">
              ...
              <h3 className="text-center mt-4">Enter your name and surname to register</h3>
              <div className="grid mb-8 mt-4 grid-flow-row">
                <input className="p-2 mb-4 text-black rounded-md" placeholder="Enter your Name" onChange={handleNameChange} />
                <input className="p-2 mb-4 text-black rounded-md" placeholder="Enter your Surname" onChange={handleLastNameChange} />
                <GradientButton
                  title="Register"
                  onClick={() => {
